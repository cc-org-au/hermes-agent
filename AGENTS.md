...
- **Sudo on the mini (not the droplet):** **`ssh_operator.sh`** does **not** use **`sudo -S`** or a password from **`~/.env/.env`**. It uses **`ssh -tt`**, **`RequestTTY=force`**, reattaches stdin/stdout to **`/dev/tty`** on the remote (so **`bash -lc`** does not leave **`sudo`** without a TTY), then **`sudo -k`** and **`sudo -v`** (when your session is interactive or **`HERMES_OPERATOR_INTERACTIVE=1`**) so **`sudo`** must authenticate. If you still never see a password prompt, check the mini’s **`/etc/sudoers`** for **`NOPASSWD`**, and **`/etc/pam.d/sudo`** — **`pam_tid`** can satisfy **`sudo`** via Touch ID without typing. Unattended SSH: set **`HERMES_OPERATOR_SKIP_SUDO_VERIFY=1`** (and often **`HERMES_OPERATOR_SSH_NO_TTY=1`**). To keep a cached sudo timestamp, **`HERMES_OPERATOR_SKIP_SUDO_K=1`**.
