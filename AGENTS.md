...
- **Hermes on the mini:** The repo clone is **not** enough — you need a **venv** and install: **`cd ~/hermes-agent && python3 -m venv venv && ./venv/bin/pip install -U pip && ./venv/bin/pip install -e .`** (Python **≥3.11**). Then **`./venv/bin/hermes`** or **`hermes`** after **`PATH`** / **`activate`**. **`hermes setup`** is an **interactive** wizard — run it in a **real terminal** with a TTY (**`ssh -tt`**, or bare **`operator`** from Terminal.app). If stdin is not a TTY, the wizard prints non-interactive guidance; use **`hermes config set …`** or env vars instead.
