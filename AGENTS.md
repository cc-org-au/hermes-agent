...
- **Remote sudo password:** put **`SSH_SUDO_PASSWORD`** in the same **`~/.env/.env`** as **`SSH_*`**. Then **`hermes … droplet`** uses **`sudo -S`** on the server (no interactive sudo prompt). If unset, you type sudo on the **remote** TTY after SSH.
