...
**Shell outside the repo (Mac/Linux):** To make **`hermes`** always use this checkout’s venv and shim (including **`hermes … droplet`**), source **`scripts/shell/hermes-env.sh`** from **`~/.zshrc`** / **`~/.bashrc`** after setting **`HERMES_AGENT_REPO`** if your clone is not **`$HOME/hermes-agent`**. That defines **`hermes()`** (delegates to **`scripts/hermes`**) and **`droplet()`** (runs **`scripts/ssh_droplet_user.sh`** — interactive login as **`hermesuser`**; needs **`~/.env/.env`** + key). On the VPS, **`agent-droplet`** runs **`./venv/bin/python -m hermes_cli.main`** and exports **`VIRTUAL_ENV`** + **`PATH`** so child processes keep using that venv.
