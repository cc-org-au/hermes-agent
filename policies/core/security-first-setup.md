...
> **Governance read order** — step 1 of 60 in the canonical `policies/` sequence (layer map & tables: [`README.md`](../README.md)).
...
**REM-001 (management port exposure):** Prefer **firewall restriction** of the SSH management port to **Tailscale** (CGNAT IPv4 `100.64.0.0/10`, IPv6 ULA `fd7a:115c:a1e0::/48`, and/or interface `tailscale0`) over rebinding `sshd` to loopback alone when that would be the only remote path. Repo script: `scripts/harden_ssh_management_port_tailscale.sh` (dry-run by default; `REM001_APPLY=1` requires root). Apply arms a **timed rollback** (default **180s**, PID in `/tmp/rem001-arm-rollback.pid`); cancel with `sudo kill "$(cat /tmp/rem001-arm-rollback.pid)"` after a successful new Tailscale SSH test. **Immediate revert:** `sudo bash …/harden_ssh_management_port_tailscale.sh rollback`. Persist rules only after cancel + verification (e.g. `netfilter-persistent save`).
