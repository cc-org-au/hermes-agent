...
> **Governance read order** — step 10 of 60 in the canonical `policies/` sequence (layer map & tables: [`README.md`](../README.md)).
...
| REM-006 | Security foundation roles (9) | **Registers:** `scripts/templates/rem_operations/SECURITY_SUBAGENTS_REGISTER.md` → materialized to `workspace/operations/` via `scripts/materialize_rem_operations.sh` (also invoked from `materialize_policies_into_hermes_home.sh`). **Unfinished:** no per-role Hermes delegate profiles or cron automation shipped; operators use Chief sessions + canonical security standard. |
