...
| **Operational truth** | `AGENT_HOME/workspace/memory/runtime/operations/` | Cross-project **registers**, audits, and governance YAML. Not a substitute for canonical policies. |

Keep canonical policy sources under `policies/` (except `generated/`) auditable and small. **Numbered domain policies** live in `workspace/memory/governance/source/standards/`. Put volume and churn in `AGENT_HOME/workspace/memory/runtime/operations/` or `AGENT_HOME/workspace/memory/governance/source/generated/`.
...
- Operational registers live under `AGENT_HOME/workspace/memory/runtime/operations/`. **Project** trees (charter, memory, archival) live under `AGENT_HOME/workspace/memory/knowledge/projects/`.
...
## 2. Workspace layout (mandatory shape)

At `AGENT_HOME/workspace/memory/`:
...
**Archival rule:** Any agent with a project affiliation **must** write to `AGENT_HOME/workspace/memory/knowledge/projects/<slug>/memory/archival/` continuously: dated filenames (`YYYY-MM-DD_topic.md` or sequential `0001_topic.md`), append-style entries for decisions, evidence pointers (not secrets), blockers, and handoffs. **Active recall:** orchestrators and project leads **must** consult recent archival files when resuming work or answering status questions.
...
2. The file lands in `workspace/memory/governance/source/generated/` (or runtime editable mirror under `AGENT_HOME/workspace/memory/governance/source/generated/`) **or** `AGENT_HOME/workspace/memory/knowledge/projects/<slug>/generated/` per scope.  
...
- Treat `AGENT_HOME/workspace/memory/runtime/operations/` as sensitive operational data; restrict filesystem ACLs in real deployments.  
