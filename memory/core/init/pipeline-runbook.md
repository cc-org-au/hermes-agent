...
| REM-006 | Security foundation roles (9) | **Registers:** `scripts/templates/rem_operations/SECURITY_SUBAGENTS_REGISTER.md` → materialized to `workspace/memory/runtime/operations/` via `scripts/core/materialize_rem_operations.sh` (also invoked from `materialize_policies_into_hermes_home.sh`). **Unfinished:** no per-role Hermes delegate profiles or cron automation shipped; operators use Chief sessions + canonical security standard. |
...
| REM-009 | Project Lead `agentic-company` (AG-005) | **Register:** `PROJECT_LEADS_REGISTER.md` + `memory/knowledge/projects/agentic-company/README.md`. **Unfinished:** optional dedicated delegate profile / long-lived agent process not created automatically. |
